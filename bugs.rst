...
이 목록을 참조해야 합니다.
 ...
 - 컨트랙트가 릴리즈된 버전이 아닌 알파테스트 컴파일러 버전(Nightly compiler version)
   으로 컴파일 된 경우 다소 의심스럽습니다. 이 목록은 릴리즈 되지 않았거나
   알파테스트 버전을 트래킹하지 않습니다.
 - 또한 컨트랙트를 작성한 시점에서 가장 최신 버전이 아닌 컴파일러로 컴파일 되었다면
   다소 의심스럽습니다. 다른 컨트랙트들에 의해 만들어진 컨트랙트의 경우, you
   have to follow the creation chain back to a transaction and use the date of
   that transaction as creation data.
